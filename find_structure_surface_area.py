...
def get_surfaceArea(normals):
    return np.sum(np.sqrt(np.sum(np.multiply(normals,normals),axis=1)))

# This function calls the other functions given ID and finds volume
def calculate_surfaceArea(ID):
    ...
    for struct in hemi_structs:
        try:
            ...
            struct_vols.append({'Image_Series_ID': ID, 'Structure_ID': struct,\
             'Surface_Area': surface_area, 'Side': 'l'})
        except:
            pass
    ...
    for struct in medi_structs:
        try:
            ...
            struct_vols.append({'Image_Series_ID': ID, 'Structure_ID': struct,\
             'Surface_Area': surface_area, 'Side': 'm'})
        except:
            pass
    ...
    while(save):
        count += 1
        try:
            df.to_sql('surface_areas',conn,if_exists='append')
            save = False
        except:
            count+=1
            if count>20:
